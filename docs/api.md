...
В режиме `url` ссылка берётся как есть: фильтры, регион и сортировка
переносятся в JSON API локально. `presentationType=serp` и `sort=date` из
внешнего конвертера отбрасываются — иначе JSON отдаёт сплошное «Продвинуто»,
хотя на сайте по той же ссылке обычные объявления. Если в ссылке уже задан
