...
В режиме `url` ссылка берётся как есть. Если в ней уже задан фильтр моделей,
повторно фильтровать по названию не нужно — сервер это определяет сам и
сообщает флагом `iphone_models_in_url`.
